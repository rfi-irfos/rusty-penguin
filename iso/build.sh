...
echo ""
echo "  Run (with persistent disk — files survive reboots):"
echo "    bash launch.sh"
echo ""
echo "  Or by hand, attaching a SATA disk for RPFS persistence:"
echo "    qemu-img create -f raw rusty-penguin-disk.img 256M"
echo "    qemu-system-x86_64 -machine q35 -cdrom $OUT_ISO -m 512M \\"
echo "      -drive id=hd0,file=rusty-penguin-disk.img,format=raw,if=none \\"
echo "      -device ich9-ahci,id=ahci -device ide-hd,drive=hd0,bus=ahci.0"
echo ""
