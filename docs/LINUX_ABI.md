...
| # | Brick | Status |
|---|-------|--------|
| 1 | Run a **freestanding** static Linux ELF (raw `write`+`exit_group`), SysV initial stack (argc/argv/envp/auxv), per-process ABI mode, Linux syscall dispatch | ✅ **DONE 2026-05-29** — proof `docs/logs/linux-abi-brick1-serial.txt`; harness `iso/build-linux-abi-test.sh` |
| 2 | A real **static glibc** binary runs natively — `printf` + working **TLS** (`__thread`) verified, full startup syscall sequence (`brk`/`arch_prctl`/`set_tid_address`/`fstat`/`prlimit64`/`getrandom`/…), clean exit via `exit_group`. Proof `docs/logs/linux-abi-brick2-serial.txt` | ✅ **DONE 2026-05-29** |
...
| 3 | **File syscalls + file-backed mmap** against the ramfs: `openat`/`open`/`read`/`pread64`/`lseek`/`close`/`fstat`/`newfstatat`, and `mmap` with a real fd (copies file segments). Verified: a glibc program opens a ramfs file and reads its bytes. Proof `docs/logs/linux-abi-brick3-fileio-serial.txt` | ✅ **DONE 2026-05-29** (groundwork for dynamic linking) |
| 3b | **Real per-process VMM** (demand paging, proper mmap/munmap, reclaim) — current arenas are crude bumps in the identity map | ❌ |
| 4 | **Dynamic linking** (`ld-linux`): ✅ **DONE 2026-05-29** — a dynamically-linked glibc binary runs natively. Kernel loads `PT_INTERP` (`ld-linux-x86-64.so.2`) at `AT_BASE`, ships ld.so + `libc.so.6` in the initrd; ld.so openat/mmap/relocates the program + maps libc. Fixes: load ET_DYN with a bias (`load_bias`), honor **MAP_FIXED** (or library segments scatter), `access`/`faccessat`, and **unique `st_dev`/`st_ino`** (ld.so dedups objects by inode — identical inodes made it skip mapping libc). Proof `docs/logs/linux-abi-brick4-dynamic-serial.txt` |
